...
fn evaluate_item(monkeys: &mut Vec<Monkey>, chosen: usize) {
...
fn evaluate_turn(monkeys: &mut Vec<Monkey>, chosen: usize) {
    while monkeys[chosen].items.len() > 0 {
        evaluate_item(monkeys, chosen);
    }
}

fn evaluate_round(monkeys: &mut Vec<Monkey>) {
    let monkey_count = monkeys.len();

    for current_monkey in 0..monkey_count {
        evaluate_turn(monkeys, current_monkey);
    }
}
