...
impl LazyMap {
    ...
    fn gen_tile_next(&self, tile: &(usize, usize, usize)) -> Vec<(usize, usize, usize)> {
        let mut result = vec![
...
fn evaluate_shortest_path(map: &mut LazyMap, start: Position, goal: Position, time: usize) -> usize {
    let mut open_set = vec![(start.0, start.1, time)];
    ...
    while ! open_set.is_empty() {
        ...
        for next in map.gen_tile_next(&current).into_iter().filter(|t| map.is_accessible(t.0, t.1, t.2)) {
...
fn main() {
    ...
    let mut time = 0;
    time = evaluate_shortest_path(&mut map, start, goal, time);
    time = evaluate_shortest_path(&mut map, goal, start, time);
    time = evaluate_shortest_path(&mut map, start, goal, time);
    println!("shortest path: {}", time);
}
